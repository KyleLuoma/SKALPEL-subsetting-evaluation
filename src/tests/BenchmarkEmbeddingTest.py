...
def init_test():
    bm_embed = BenchmarkEmbedding(
        benchmark_name="abstract", 
        verbose=True, 
        db_host_profile=DB_HOST_PROFILE, 
        db_host=DB_HOST,
...
def get_identifier_ambiguity_problem_results_test():
    ...
    results = bm_embed.get_identifier_ambiguity_problem_results(
        database_name="NTSB",
        question_number=1,
        naturalness="Native"
    )
    for item in results.word_nl_matches:
        print(item.word_nl)
        for att in item.matching_attributes:
            print(att)
