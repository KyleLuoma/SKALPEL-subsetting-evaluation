...
def execute_query_with_timeout_test():
    ...
    result = bm.execute_query(
        query=query,
        database="AdventureWorks",
        query_timeout=0.0000000000000000000000000000001
    )
    return result.error_message is not None and "Query execution" in result.error_message


def execute_query_with_no_timeout_test():
    query = "select NAME, productnumber from product limit 5;"
    bm = Spider2NlSqlBenchmark()
    result = bm.execute_query(
        query=query,
